...
| `BGV_Requests` | `ReferenceGuardrailLastEmailNormalized` | N/A | Updated by `BGV_4` with the submitted normalized HR/reference email used in the guardrail check |

Notes:
- In `BGV_4`, prefill source order is `BGV_FormData` first, then fallback to `BGV_Candidates`/`BGV_Requests`.
- In `BGV_4`, the guardrail lookup is based on the submitted normalized HR/reference email, not on the technical fallback mailbox address used to prevent send-action runtime failures.
...
| `r2d39255c2449439096683ca0e39241b0` | Company details accurate | constant `Yes` prefill |
...
| Derived runtime values | Scoring output | `BGV_Requests.Severity/Value`, `Outcome/Value`, `PEV Checks`, `Notes`, `VerificationStatus/Value='Responded'`, `ResponseReceivedAt=utcNow()` | `Update_item_-_of_BGV_Request` |
...
| `rafe3ada4157c49fb9e555cd0fb53bd59` | Re-employ check (`== 'No'`) -> High severity path |
| `r5f7ebc3390bc4699b160504c65254c3e` | Reason text appended to re-employ notification body |
| `r9594fab1bfa04c90883b1dffd7f4549e` | Employment-details accuracy check (`== 'No'`) -> Medium severity when severity still empty |
| `r2d39255c2449439096683ca0e39241b0` | Company-details accuracy check (`== 'No'`) -> Low severity when severity still empty |
...
| `varOutcome` | empty string | `Initialize_variable_-_Outcome` |
...
| 11 | Reason For Leaving | `r513ad5ab3a14453286bdb910820985ec` | Prefill + Stored | Prefilled when value is available and submitted response is stored in `BGV_FormData.F2_ReasonForLeaving` |
...
- HR `Reason For Leaving` (`r513ad5ab3a14453286bdb910820985ec`) is now included in prefill when source data exists.
