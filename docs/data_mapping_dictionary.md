...
| `BGV_6_HRReminderAndEscalation` | `BGV_Requests` | Reads rows with `HRRequestSentAt` present and no `ResponseReceivedAt`; uses `Reminder1At`, `Reminder2At`, `Reminder3At`; updates timestamps and sets `VerificationStatus='Reminder 1 Sent'/'Reminder 2 Sent'/'Reminder 3 Sent'`. |
